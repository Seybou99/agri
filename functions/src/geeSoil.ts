 ...
 * pour un point (lat, lng). Utilisé par la Cloud Function getSoilFromGEE.
 *
...
function ensureEEInitialized(credentialsPath?: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const keyPath =
      ...
      path.join(__dirname, '..', 'keys', 'gee-sa.json');
    let key: object;
    try {
      const raw = fs.readFileSync(keyPath, 'utf8');
      key = JSON.parse(raw) as object;
    } catch (e) {
      reject(new Error(`GEE: impossible de charger la clé ${keyPath}: ${e}`));
      return;
    }
...
export async function fetchSoilFromGEE(
  ...
  await ensureEEInitialized(credentialsPath);
